...
def create_app():
    ...
    # Respeta X-Forwarded-* cuando la app está detrás de Nginx/reverse proxy.
    ...
    logger.debug(
        f"JWT configurado - Access token: {config.JWT_ACCESS_TOKEN_EXPIRES}min, Refresh: {config.JWT_REFRESH_TOKEN_EXPIRES}días"
    )
    ...
    rate_limit_service.init_app(app, config)
    ...
    def transcription_validator():
        ...
        logger.info(
            f"Usuario autenticado accediendo a validador: {request.current_user['username']}"
        )
    ...
    def admin_page():
        ...
        try:
            ...
            if payload.get("role") != "admin":
                logger.warning(
                    f"Admin access denied - user {payload.get('username')} is not admin"
                )
                return redirect("/login")
    ...
    def admin_dashboard():
        ...
        try:
            ...
            if payload.get("role") != "admin":
                logger.warning(
                    f"Admin dashboard access denied - user {payload.get('username')} is not admin"
                )
                return redirect("/transcription/validator")
    ...
    def login():
        """Autentica usuario y escribe cookies JWT HttpOnly."""
        logger.info("=" * 80)
        ...
        try:
            ...
            logger.info(
                f"👤 Username recibido: '{username}' (longitud={len(username)})"
            )
            logger.info(
                f"🔑 Password recibido: {'*' * min(len(password), 8) if password else 'EMPTY'} (longitud={len(password)})"
            )
            ...
            if user:
                ...
                logger.info("🔐 Verificando hash de contraseña...")
                pwd_valid = user.check_password(password)
                logger.info(f"   └─ Contraseña válida: {pwd_valid}")
                if not pwd_valid:
                    logger.warning(
                        "   └─ ❌ Hash no coincide con password proporcionado"
                    )
            else:
                logger.warning("   └─ ❌ Usuario NO encontrado en BD")
            ...
            if not user or not user.check_password(password):
                ...
                return (
                    jsonify(
                        {"error": "Usuario o contraseña incorrectos", "success": False}
                    ),
                    401,
                )
            ...
            # Generar tokens JWT
            logger.info(
                f"✅ LOGIN EXITOSO - Generando tokens JWT para: {user.username} (ID: {user.id})"
            )
            ...
            return (
                jsonify(
                    {"error": f"Error interno del servidor: {str(e)}", "success": False}
                ),
                500,
            )
    ...
    @rate_limit_service.limit_api_call
    def refresh():
        """Renueva access token usando refresh token desde cookie HttpOnly."""
        try:
            refresh_token = jwt_service.get_refresh_token_from_cookie()
            if not refresh_token:
                logger.debug("POST /refresh - No refresh token provided")
            ...
            if not user:
                logger.warning(
                    f"POST /refresh - User not found: {payload.get('user_id')}"
                )
            ...
            jwt_service.set_auth_cookies(response, access_token)
            return response, 200
        except Exception as e:
            logger.error(f"Error en POST /refresh: {str(e)}")
    ...
    logger.info(f"Base de datos: {config.DATABASE_URL.split(':', 1)[0].upper()}")
    logger.info("Autenticación: JWT en cookies HttpOnly")
...
if __name__ == "__main__":
    try:
        ...
        logger.info(
            f"Ejecutando servidor en {config.HOST}:{config.PORT} (debug={config.DEBUG})"
        )
