...
class Config:
    ...
    TRANSCRIPT_SOURCE_PATH: Optional[str] = (
        None  # ← JSON fuente (si difiere del directorio de proyecto)
    )
    ...
    def from_env(cls):
        ...
        config = cls(
            TRANSCRIPTION_PROJECTS_PATH=os.getenv(
                "TRANSCRIPTION_PROJECTS_PATH", cls.TRANSCRIPTION_PROJECTS_PATH
            ),
            TRANSCRIPT_SOURCE_PATH=os.getenv("TRANSCRIPT_SOURCE_PATH"),
            AUDIO_FILES_PATH=os.getenv("AUDIO_FILES_PATH", cls.AUDIO_FILES_PATH),
            UPLOADS_PATH=os.getenv("UPLOADS_PATH", cls.UPLOADS_PATH),
            AUTOSAVE_INTERVAL=int(
                os.getenv("AUTOSAVE_INTERVAL", cls.AUTOSAVE_INTERVAL)
            ),
            DEBUG=os.getenv("DEBUG", "False" if env == "production" else "True").lower()
            == "true",
            HOST=os.getenv("HOST", cls.HOST),
            PORT=int(os.getenv("PORT", cls.PORT)),
            WORKERS=int(
                os.getenv("WORKERS", cls.WORKERS if env == "production" else 1)
            ),
            ...
            JWT_ACCESS_TOKEN_EXPIRES=int(
                os.getenv("JWT_ACCESS_TOKEN_EXPIRES", cls.JWT_ACCESS_TOKEN_EXPIRES)
            ),
            JWT_REFRESH_TOKEN_EXPIRES=int(
                os.getenv("JWT_REFRESH_TOKEN_EXPIRES", cls.JWT_REFRESH_TOKEN_EXPIRES)
            ),
            ...
            DB_CONNECT_TIMEOUT=int(
                os.getenv("DB_CONNECT_TIMEOUT", cls.DB_CONNECT_TIMEOUT)
            ),
            ...
            HEALTH_CHECK_INTERVAL=int(
                os.getenv("HEALTH_CHECK_INTERVAL", cls.HEALTH_CHECK_INTERVAL)
            ),
            DISK_SPACE_WARNING_THRESHOLD=int(
                os.getenv(
                    "DISK_SPACE_WARNING_THRESHOLD", cls.DISK_SPACE_WARNING_THRESHOLD
                )
            ),
            DB_CONNECTION_WARNING_THRESHOLD=float(
                os.getenv(
                    "DB_CONNECTION_WARNING_THRESHOLD",
                    cls.DB_CONNECTION_WARNING_THRESHOLD,
                )
            ),
            API_VERSION=os.getenv("API_VERSION", cls.API_VERSION),
            API_DEPRECATION_HEADERS=os.getenv("API_DEPRECATION_HEADERS", "True").lower()
            == "true",
            TELEGRAM_BOT_TOKEN=os.getenv("TELEGRAM_BOT_TOKEN"),
            TELEGRAM_ADMIN_CHAT_ID=os.getenv("TELEGRAM_ADMIN_CHAT_ID"),
            NOTIFICATION_RETRY_ATTEMPTS=int(
                os.getenv(
                    "NOTIFICATION_RETRY_ATTEMPTS", cls.NOTIFICATION_RETRY_ATTEMPTS
                )
            ),
            REDIS_URL=os.getenv("REDIS_URL", cls.REDIS_URL),
            SESSION_TYPE=os.getenv("SESSION_TYPE", cls.SESSION_TYPE),
            PERMANENT_SESSION_LIFETIME=int(
                os.getenv("PERMANENT_SESSION_LIFETIME", cls.PERMANENT_SESSION_LIFETIME)
            ),
        )
    ...
    def validate_production_config(self):
        ...
        if self.is_production():
            ...
            if not self.JWT_SECRET_KEY:
                ...
                errors.append(
                    "JWT_SECRET_KEY debe tener al menos 32 caracteres (32+ aleatorios)"
                )
            ...
            if not self.SECRET_KEY:
                ...
                errors.append(
                    "SECRET_KEY debe tener al menos 32 caracteres (32+ aleatorios)"
                )

            # Validar base de datos
            if "sqlite" in self.DATABASE_URL.lower():
                errors.append(
                    "SQLite NO es recomendado para producción. Usa PostgreSQL o MySQL"
                )
            ...
            if self.WORKERS < 2:
                errors.append(
                    f"Se recomienda mínimo 2 workers en producción (actual: {self.WORKERS})"
                )
    ...
    def get_db_engine_config(self) -> dict:
        ...
        if (
            ...
            or "mysql" in self.DATABASE_URL.lower()
        ):
    ...
    def get_transcript_source_path(self) -> str:
        """Obtiene la ruta absoluta a los JSON fuente de transcripción."""
        source_path = self.TRANSCRIPT_SOURCE_PATH or self.TRANSCRIPTION_PROJECTS_PATH
        if os.path.isabs(source_path):
            return source_path

        app_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        return os.path.join(app_root, source_path)
    ...
    def ensure_directories_exist(self):
        ...
        directories = [
            ...
            self.get_transcript_source_path(),
...
Transcript Source:  {self.get_transcript_source_path()}
