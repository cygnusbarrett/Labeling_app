...
from services.jwt_service import admin_required, jwt_required, jwt_service
from services.notification_service import notification_service
from services.security_utils import (SecurityUtils, rate_limit,
                                     validate_json_input)
...
def login():
    ...
    try:
        ...
        password = data[
            "password"
        ]  # No validar password para preservar caracteres especiales
        ...
        if user:
            ...
            logger.info(
                f"Login exitoso para usuario {username} (ID: {user.id}, rol: {user.role})"
            )
            ...
            return jsonify(
                {
                    ...
                    "user": user.to_dict(),
                    "expires_in": jwt_service.access_token_expire_minutes
                    * 60,  # en segundos
                }
            )
        else:
...
def refresh_token():
    ...
    try:
        ...
        if not user:
            logger.warning(
                f"Usuario no encontrado para refresh token: {payload['user_id']}"
            )
...
def get_next_task():
    ...
    if task_data:
        ...
        logger.info(
            f"Tarea asignada a {username}: anotación {annotation.id}, imagen {image.id}"
        )
        ...
        try:
            notification_sent = notification_service.send_no_tasks_notification(
                user_id, username
            )
            if notification_sent:
                logger.info(
                    f"Notificación de 'sin tareas' enviada al admin para usuario: {username}"
                )
            else:
                logger.debug(
                    f"Notificación de 'sin tareas' no enviada (anti-spam o error) para usuario: {username}"
                )
        except Exception as e:
            logger.error(
                f"Error enviando notificación de 'sin tareas' para usuario {username}: {e}"
            )
...
def get_pending_preview():
    ...
    logger.debug(
        f"Solicitando vista previa de tareas pendientes para {username} (límite: {limit})"
    )

    pending_tasks = db_service.get_pending_tasks_preview(user_id, limit)

    logger.debug(
        f"Vista previa obtenida para {username}: {len(pending_tasks)} tareas pendientes"
    )

    return jsonify(
        {
            "pending": [
                {"annotation_id": annotation.id, "image_id": image.id}
...
def load_specific_task(annotation_id):
    ...
    if task_data:
        annotation, image = task_data
        logger.info(
            f"Tarea específica cargada para {username}: anotación {annotation_id}"
        )
        return jsonify(
            {
                "annotation_id": annotation.id,
        ...
        logger.warning(
            f"Tarea específica no encontrada: {annotation_id} para usuario {username}"
        )
...
def update_annotation(annotation_id):
    ...
    try:
        ...
        logger.info(
            f"Actualizando anotación {annotation_id} para {username}: status={status}"
        )
        ...
        success = db_service.update_annotation(
            annotation_id, user_id, status, corrected_text
        )

        if success:
            logger.info(
                f"Anotación {annotation_id} actualizada exitosamente por {username} a {status}"
            )
            return jsonify({"success": True, "message": f"Annotation {status}"})
        else:
            logger.warning(
                f"Fallo actualizando anotación {annotation_id} para {username}: no encontrada o no autorizada"
            )
            return jsonify({"error": "Annotation not found or not authorized"}), 404

    except ValueError as e:
        logger.warning(
            f"Error de validación actualizando anotación {annotation_id}: {e}"
        )
        return jsonify({"error": str(e)}), 400
    except Exception as e:
...
def admin_update_annotation(annotation_id):
    ...
    try:
        ...
        logger.info(
            f"[ADMIN] Actualizando anotación {annotation_id} por {admin_username}: status={status}"
        )
        ...
        if status not in valid_statuses:
            logger.warning(
                f"[ADMIN] Estado inválido para anotación {annotation_id}: {status}"
            )
            return (
                jsonify({"error": f"Invalid status. Must be one of: {valid_statuses}"}),
                400,
        ...
        success = db_service.admin_update_annotation(
            annotation_id, status, corrected_text
        )

        if success:
            logger.info(
                f"[ADMIN] Anotación {annotation_id} actualizada exitosamente a {status} por {admin_username}"
            )
            return jsonify({"success": True, "message": f"Annotation {status}"})
        else:
            logger.warning(
                f"[ADMIN] Fallo actualizando anotación {annotation_id}: no encontrada"
            )
            return jsonify({"error": "Annotation not found"}), 404

    except ValueError as e:
        logger.warning(
            f"[ADMIN] Error de validación actualizando anotación {annotation_id}: {e}"
        )
        return jsonify({"error": str(e)}), 400
    except Exception as e:
        logger.error(
            f"[ADMIN] Error interno actualizando anotación {annotation_id}: {e}"
        )
        return jsonify({"error": "Failed to update annotation"}), 500
...
def get_annotation(annotation_id):
    ...
    if annotation_data:
        ...
        logger.warning(
            f"Anotación {annotation_id} no encontrada para usuario {username}"
        )
        return jsonify({"error": "Annotation not found"}), 404
...
def create_assignments():
    ...
    try:
        ...
        logger.info(
            f"Admin {admin_username} creando asignaciones: {len(user_ids)} usuarios, {len(image_ids)} imágenes"
        )

        # Validar que las listas no estén vacías
        if not user_ids or not image_ids:
            logger.warning(
                f"Admin {admin_username} intentó crear asignaciones con listas vacías"
            )
        ...
        logger.info(
            f"Admin {admin_username} creó {assignments} asignaciones exitosamente"
        )
...
@admin_required
@validate_json_input(
    required_fields=["count", "priority_unannotated"], optional_fields=["user_id"]
)
def create_auto_assignments():
    ...
    try:
        ...
        logger.info(
            f"Admin {admin_username} creando {count} asignaciones automáticas"
            + (f" para usuario {user_id}" if user_id else "")
        )

        if count <= 0 or count > 50000:  # Límite de seguridad
            logger.warning(
                f"Admin {admin_username} intentó crear {count} asignaciones (fuera de rango)"
            )
            return jsonify({"error": "Count must be between 1 and 50000"}), 400

        assignments = db_service.assign_random_tasks(
            user_id, count, priority_unannotated
        )

        logger.info(
            f"Admin {admin_username} creó {assignments} asignaciones automáticas exitosamente"
        )
        ...
        logger.warning(
            f"Error de validación en asignaciones automáticas para admin {admin_username}: {e}"
        )
        return jsonify({"error": str(e)}), 400
    except Exception as e:
        logger.error(
            f"Error creando asignaciones automáticas para admin {admin_username}: {e}"
        )
...
def get_image_annotations(image_id):
    ...
    logger.debug(
        f"Admin {admin_username} solicitando anotaciones para imagen {image_id}"
    )

    annotations = db_service.get_image_annotations(image_id)

    logger.debug(
        f"Admin {admin_username} obtuvo {len(annotations)} anotaciones para imagen {image_id}"
    )
...
def get_all_images():
    ...
    return jsonify({"images": images})  # Ya viene formateado como lista de diccionarios
...
def create_user():
    ...
    try:
        ...
        logger.info(
            f"Admin {admin_username} creando usuario: {username} con rol {role}"
        )

        # Validar fortaleza de contraseña
        if not security.validate_password_strength(password):
            logger.warning(
                f"Admin {admin_username} intentó crear usuario {username} con contraseña débil"
            )
            return (
                jsonify(
                    {
        ...
        if role not in ["annotator", "admin"]:
            logger.warning(
                f"Admin {admin_username} intentó crear usuario {username} con rol inválido: {role}"
            )
        ...
        if user:
            ...
            logger.warning(
                f"Admin {admin_username} falló creando usuario {username}: username ya existe"
            )
            return jsonify({"error": "Username already exists"}), 409

    except ValueError as e:
        logger.warning(
            f"Error de validación creando usuario para admin {admin_username}: {e}"
        )
        return jsonify({"error": str(e)}), 400
    except Exception as e:
...
def create_image():
    ...
    try:
        ...
        initial_ocr_text = security.validate_input(
            data["initial_ocr_text"], max_length=2000
        )
        ...
        logger.warning(
            f"Error de validación creando imagen para admin {admin_username}: {e}"
        )
        return jsonify({"error": str(e)}), 400
    except Exception as e:
...
def get_recent_activity():
    ...
    logger.debug(
        f"Admin {admin_username} solicitando actividad reciente (límite: {limit})"
    )
...
def get_user_detailed_stats(user_id):
    ...
    logger.debug(
        f"Admin {admin_username} solicitando estadísticas detalladas para usuario {user_id}"
    )
    ...
    logger.debug(
        f"Admin {admin_username} obtuvo estadísticas detalladas para usuario {user_id}"
    )
...
def get_user_annotations(user_id):
    ...
    logger.debug(
        f"Admin {admin_username} solicitando anotaciones para usuario {user_id}"
    )

    annotations = db_service.get_user_annotations_detailed(user_id)

    logger.debug(
        f"Admin {admin_username} obtuvo {len(annotations)} anotaciones para usuario {user_id}"
    )

    return jsonify({"user_id": user_id, "annotations": annotations})


@api_bp.route(
    "/admin/users/<int:user_id>/annotations/<int:annotation_id>", methods=["DELETE"]
)
@admin_required
def delete_user_annotation(user_id, annotation_id):
    ...
    logger.info(
        f"Admin {admin_username} eliminando anotación {annotation_id} del usuario {user_id}"
    )
    ...
    if success:
        logger.info(
            f"Admin {admin_username} eliminó anotación {annotation_id} exitosamente"
        )
        return jsonify({"success": True, "message": "Annotation deleted successfully"})
    else:
        logger.warning(
            f"Admin {admin_username} falló eliminando anotación {annotation_id}"
        )
...
def bulk_delete_user_annotations(user_id):
    ...
    try:
        ...
        if not isinstance(statuses, list) or not all(
            status in ["pending", "corrected", "approved", "discarded"]
            for status in statuses
        ):
            return jsonify({"error": "Invalid statuses provided"}), 400

        logger.info(
            f"Admin {admin_username} eliminando anotaciones del usuario {user_id} con estados {statuses}"
        )

        deleted_count = db_service.delete_user_annotations_by_status(user_id, statuses)

        logger.info(
            f"Admin {admin_username} eliminó {deleted_count} anotaciones del usuario {user_id}"
        )
        ...
        logger.error(
            f"Error eliminando anotaciones masivamente para admin {admin_username}: {e}"
        )
...
@admin_required
@validate_json_input(
    required_fields=["to_user_id"],
    optional_fields=["include_pending", "include_reviewed"],
)
def transfer_user_annotations(from_user_id):
    ...
    try:
        ...
        if from_user_id == to_user_id:
            return (
                jsonify({"error": "Source and destination users cannot be the same"}),
                400,
            )

        logger.info(
            f"Admin {admin_username} transfiriendo anotaciones de usuario {from_user_id} a {to_user_id}"
        )
        ...
        if result["success"]:
            logger.info(
                f"Admin {admin_username} transfirió {result['transferred']} anotaciones exitosamente"
            )
            return jsonify(result)
        else:
            logger.warning(
                f"Admin {admin_username} falló transfiriendo anotaciones: {result['error']}"
            )
            return jsonify(result), 400

    except ValueError as e:
        logger.warning(
            f"Error de validación transfiriendo anotaciones para admin {admin_username}: {e}"
        )
        return jsonify({"error": "Invalid user ID provided"}), 400
    except Exception as e:
        logger.error(
            f"Error interno transfiriendo anotaciones para admin {admin_username}: {e}"
        )
...
def get_quality_control_annotations():
    ...
    logger.debug(
        f"Admin {admin_username} obtuvo {len(quality_data)} discrepancias para control de calidad"
    )

    return jsonify(
        {"quality_control_data": quality_data, "total_discrepancies": len(quality_data)}
    )
...
def consolidate_annotation():
    ...
    try:
        ...
        logger.info(
            f"Admin {admin_username} consolidando anotación: user={user_annotation_id}, admin={admin_annotation_id}"
        )

        success = db_service.consolidate_annotation(
            user_annotation_id, admin_annotation_id
        )

        if success:
            logger.info(f"Admin {admin_username} consolidó anotación exitosamente")
            return jsonify(
                {"success": True, "message": "Annotation consolidated successfully"}
            )
        else:
...
def get_users_agreement_stats():
    ...
    try:
        ...
        logger.debug(
            f"Admin {admin_username} obtuvo estadísticas de agreement para {len(agreement_stats)} usuarios"
        )
...
def get_notifications_status():
    ...
    try:
        ...
        logger.debug(
            f"Admin {admin_username} obtuvo estado de notificaciones para {len(notification_statuses)} usuarios"
        )

        return jsonify(
            {"success": True, "notification_statuses": notification_statuses}
        )
...
def reset_user_notification_status(user_id):
    ...
    logger.debug(
        f"Admin {admin_username} reseteando estado de notificación para usuario ID: {user_id}"
    )
    ...
    try:
        ...
        logger.info(
            f"Admin {admin_username} reseteó estado de notificación para usuario {user.username} (ID: {user_id})"
        )
...
def test_notification():
    ...
    custom_message = data.get(
        "message",
        "Esta es una notificación de prueba desde la aplicación de anotaciones.",
    )
    ...
    try:
        ...
        if success:
            logger.info(
                f"Admin {admin_username} envió notificación de prueba exitosamente"
            )
            return jsonify(
                {
                    "success": True,
            ...
            logger.warning(
                f"Error enviando notificación de prueba para admin {admin_username}"
            )
            return (
                jsonify(
...
def get_notification_config():
    ...
    try:
        config = {
            ...
            "min_notification_interval_hours": notification_service.min_notification_interval
            / 3600,
            "notification_timeout_seconds": notification_service.notification_timeout,
            "notification_timeout_hours": notification_service.notification_timeout
            / 3600,
            "telegram_configured": bool(
                os.getenv("TELEGRAM_BOT_TOKEN")
                and os.getenv("TELEGRAM_BOT_TOKEN") != "REPLACE_WITH_BOTFATHER_TOKEN"
                and os.getenv("TELEGRAM_ADMIN_CHAT_ID")
                and os.getenv("TELEGRAM_ADMIN_CHAT_ID") != "REPLACE_WITH_CHAT_ID"
            ),
        }
...
def export_annotations():
    ...
    try:
        ...
        logger.info(
            f"Admin {admin_username} exportó {total_annotations} anotaciones de {total_images} imágenes"
        )
