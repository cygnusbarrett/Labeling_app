...
def main():
    ...
    try:
        ...
        # Buscar automáticamente archivos de audio y .json que coincidan
        # por nombre base.
        ...
        if not audio_files:
            raise FileNotFoundError(
                f"No se encontraron archivos de audio soportados en {project_dir}"
            )

        # Encontrar parejas (audio y json con mismo nombre base)
        pairs = []
        for audio_file in audio_files:
            ...
            if json_file in json_files:
                pairs.append((audio_file, json_file))

        if not pairs:
            print("\n❌ Error: No se encontraron parejas de archivos de audio y .json")
        ...
        for i, word in enumerate(first_words, 1):
            print(
                f"   {i}. [{word.speaker}] '{word.word}' (prob: {word.probability:.3f})"
            )
        ...
        print("\n🔗 Próximos pasos:")
        print(
            "   1. Asignar palabras a anotadores (usando API POST "
            "/api/v2/transcriptions/projects/...)"
        )
        print("   2. Acceder a /transcription/validator para anotar")
        print("   3. Ver estadísticas en /stats")

    except FileNotFoundError as e:
        print(f"\n❌ Error: {str(e)}")
        print("\n💡 Asegúrate de que existen archivos en:")
        print("   data/transcription_projects/memoria_1970_1990/")
        print(
            "   - Archivos de audio soportados: "
            f"{', '.join(SUPPORTED_AUDIO_EXTENSIONS)}"
        )
        print("   - Archivos .json con igual nombre base (ej: audio_name.json)")
        return 1
