...
def migrate_to_postgresql():
    ...
    logger.info("   Origen: sqlite:///labeling_app.db")
    ...
    try:
        ...
        postgres_session.bulk_insert_mappings(
            User,
            [
                {
                    "id": user.id,
            ...
            segment_mappings.append(
                {
                    ...
                    "decision_type": getattr(segment, "decision_type", None),
        ...
        logger.info(
            "     psql -U labeling_user -d labeling_db -c 'SELECT COUNT(*) FROM users;'"
        )
