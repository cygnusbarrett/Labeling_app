...
class JWTService:
    ...
    def __init__(self):
        ...
        self.secret_key = (
            ...
            else self._generate_secret_key()
        )
        ...
        logger.debug(
            f"Access token expira en: {self.access_token_expire_minutes} minutos"
        )
    ...
    def _generate_secret_key(self):
        ...
        logger.warning(
            "Generando nueva clave JWT secreta - usar JWT_SECRET_KEY en producción"
        )
        return key
    ...
    def create_access_token(self, user_id: int, username: str, role: str) -> str:
        ...
        logger.debug(
            f"Token de acceso creado para usuario {username} (ID: {user_id}, rol: {role})"
        )
        return token
    ...
    def decode_token(self, token: str) -> dict:
        ...
        try:
            ...
            logger.debug(
                f"Token decodificado exitosamente para usuario {payload.get('username', payload.get('user_id'))}"
            )
            return payload
    ...
    def get_refresh_token_from_cookie(self) -> str:
        """Extrae el refresh token desde cookie HttpOnly."""
        token = request.cookies.get(self.REFRESH_COOKIE_NAME)
        if token:
            logger.debug("Refresh token JWT extraído desde cookie")
            return token

        logger.debug("No se encontró refresh token JWT en cookie")
        return None

    def set_auth_cookies(
        self, response, access_token: str, refresh_token: str | None = None
    ):
        """Escribe cookies HttpOnly para autenticación."""
    ...
    def clear_auth_cookies(self, response):
        ...
        response.delete_cookie(self.ACCESS_COOKIE_NAME, path="/", samesite="Lax")
        response.delete_cookie(self.REFRESH_COOKIE_NAME, path="/", samesite="Lax")
        return response
...
def jwt_required(f):
    ...
    def decorated_function(*args, **kwargs):
        try:
            ...
            logger.info(
                f"Autenticación JWT exitosa para {payload.get('username')} (rol: {payload.get('role')})"
            )
    ...
    def decorated_function(*args, **kwargs):
        try:
            ...
            if payload.get("role") != "admin":
                logger.warning(
                    f"Acceso de admin denegado para {payload.get('username')} (rol: {payload.get('role')})"
                )
...
def optional_jwt(f):
    ...
    def decorated_function(*args, **kwargs):
        try:
            ...
            logger.debug(
                f"JWT opcional - usuario autenticado: {payload.get('username')}"
            )
        except Exception:
    ...
    def decorated_function(*args, **kwargs):
        try:
            ...
            if token:
                ...
                logger.debug(
                    f"Autenticación opcional exitosa para {payload.get('username')}"
                )
            else:
                request.current_user = None
