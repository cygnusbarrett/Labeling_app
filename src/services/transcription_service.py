...
from models.database import TranscriptionProject, User, Word
...
class TranscriptionService:
    ...
    def __init__(self, config=None, base_path: str = None):
        ...
        if config is not None:
            ...
            self.transcript_source_path = config.get_transcript_source_path()
        elif base_path is not None:
            # Ruta explícita (compatibilidad)
            self.base_path = base_path
            self.transcript_source_path = base_path
        else:
            # Fallback a ruta hardcodeada (desarrollo local)
            self.base_path = os.path.join(
                os.path.dirname(__file__), "..", "data", "transcription_projects"
            )
            self.transcript_source_path = self.base_path

    def _candidate_paths(self, project_id: str, filename: str) -> List[str]:
        normalized_filename = filename.lstrip("/\\")
        basename = os.path.basename(normalized_filename)
        seen = set()
        candidates = []

        for root in (self.transcript_source_path, self.base_path):
            if not root:
                continue
    ...
    def _resolve_existing_path(
        self, project_id: str, filename: str, description: str
    ) -> str:
        candidates = self._candidate_paths(project_id, filename)
        for path in candidates:
            if os.path.exists(path):
                return path

        raise FileNotFoundError(
            f"{description} no encontrado para proyecto {project_id}: {filename}. "
    ...
    def get_metadata_path(self, project_id: str) -> str:
        ...
        return self._resolve_existing_path(project_id, "metadata.json", "metadata.json")
    ...
    def load_metadata(self, project_id: str) -> Dict:
        ...
        try:
            ...
            raise json.JSONDecodeError(
                f"JSON inválido en {metadata_path}: {str(e)}", e.doc, e.pos
            )
    ...
    def load_transcript(self, project_id: str, transcript_filename: str) -> Dict:
        ...
        Raises:
            ...
            json.JSONDecodeError: Si el JSON es inválido
        """
        transcript_path = self._resolve_existing_path(
            project_id, transcript_filename, "Transcript"
        )
        ...
        try:
            ...
            raise json.JSONDecodeError(
                f"JSON inválido en {transcript_path}: {str(e)}", e.doc, e.pos
            )

    def parse_words_from_transcript(
        self, transcript_data: Dict, probability_threshold: float = 0.95
    ) -> List[Dict]:
        """
        ...
        if "words" in transcript_data:
            ...
            raise ValueError(
                "Formato de transcripción no reconocido. Se espera 'words', 'segments', o array directo."
            )
    ...
    def create_or_update_project(
        self, session, project_id: str, name: str, description: str = None
        ...
        if project:
            ...
            project = TranscriptionProject(
                id=project_id, name=name, description=description, status="active"
            )
            session.add(project)
    ...
    def import_transcript_to_db(
        ...
        words_list = self.parse_words_from_transcript(
            transcript_data, probability_threshold
        )
    ...
    def get_project_stats(
        self, session, project_id: str, annotator_id: int = None
    ) -> Dict:
        """
    ...
    def get_annotator_stats_for_project(self, session, project_id: str) -> List[Dict]:
        ...
        annotators = (
            session.query(Word.annotator_id)
